...
export function displayedDate(date: string) {
...
export function generateHorario(
  date: string,
  aulas: Array<IAula>,
  ...
  const arrayData = Array.from({ length: 33 }, (_, i) => {
    ...
    return {
      day: calcDay,
      header:
        calcDay > 0 && calcDay <= daysInMonth
          ? `Dia ${String(calcDay).padStart(2, "0")}`
          : "-",
      aulas: aulas.filter(
        (aula) =>
          aula.info.data === `${date}-${String(calcDay).padStart(2, "0")}`,
      ),
    };
  });

  const generateBody = (item: {
    day: number;
    header: string;
    aulas: Array<IAula>;
  }) => {
    const itemDate = `${date}-${String(item.day).padStart(2, "0")}`;
    if (feriados.some((el) => el.info.data === itemDate)) {
      return {
        text: "FERIADO",
        fontSize: 24,
        fillColor: "#CCCCCC",
      ...
      return item.aulas.map(
        (aula) =>
          `${displayedHour(aula.info.inicio)}-${displayedHour(aula.info.termino)}: ${aula.info.nome} (${aula.info.curso.split("/")[0].toUpperCase()})`,
      );
    }
  };
  ...
  pdfMake
    .createPdf(horarioDefinitions)
    .open(
      {},
      window.open(`Horario_${monthName}_${year}_Lapbioq.pdf`, "_blank"),
    );
}
